...
class Translator(object):
    ...
    def beam_search(self, src_seq, max_dec_step):
        ...
        with torch.no_grad():
            enc_emo_batch = src_seq["emotion_context_batch"]
            # -- Encode
            (
                enc_batch,
                _,
            ...
            """
            if config.mask_emo_unrel:
                emb_mask = self.model.embedding(src_seq["mask_input"])
                semantic_encoder_outputs = self.model.semantic_encoder(self.model.embedding(enc_batch) + emb_mask,
                                                                 self.model.embedding(enc_rel_batch), enc_rel_map,
                                                                 mask_src_semantic, mask_src_rel)
                mask_emo_path = get_emotion_path_mask_for_one_batch(enc_batch, enc_emo_batch, enc_rel_map).bool()
                mask_src_emotion = mask_src_semantic + mask_emo_path
                emotion_encoder_outputs = self.model.emotion_encoder(self.model.embedding(enc_batch), self.model.embedding(enc_rel_batch),
                                                               enc_rel_map, mask_src_emotion, mask_src_rel)
                src_enc = torch.cat((semantic_encoder_outputs, emotion_encoder_outputs), dim=1)
                mask_src = torch.cat((mask_src_semantic, mask_src_emotion), dim=2)
            """


            emb_mask = self.model.embedding(src_seq["mask_input"])
            encoder_outputs = self.model.encoder(self.model.embedding(enc_batch) + emb_mask,
                                                             self.model.embedding(enc_rel_batch), enc_rel_map,
                                                             mask_src, mask_src_rel)
            src_enc = encoder_outputs
            mask_src = mask_src
            #print("enc_batch:",enc_batch.type())
            #print("enc_rel_batch:",enc_rel_batch.type())
            #enc_rel_batch = torch.LongTensor(enc_rel_batch.numpy())
            #print("beam:",enc_rel_map.shape)
